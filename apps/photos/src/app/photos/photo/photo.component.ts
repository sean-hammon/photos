...
export class PhotoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.display$.pipe(filter((obj) => !!obj)).subscribe((display) => {
  ...
  setDisplayPhoto() {
    ...
    forkJoin([params$, path$]).subscribe(([params, path]) => {
  ...
  preloadImage(photo: PhotoDisplay) {
    ...
    img.onerror = (err: ErrorEvent) =>
      console.log(photo.photo.files.hifi.path + ': ' + err.message);
    img.src =
      environment.api + environment.imageRoot + photo.photo.files.hifi.path;
  }
